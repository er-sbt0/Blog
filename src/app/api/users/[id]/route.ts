  ...
  try {
    ...
    if (!session) {
      response.error = {
        ...
        subtitle: "Please sign in to update your profile",
      };
    ...
    if (user.id !== params.id) {
      response.error = {
        title: "Unauthorized",
        subtitle: "You are not authorized to update this profile",
      };
      return NextResponse.json(response, { status: 403 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (!session) {
      response.error = {
        title: "Unauthenticated",
        subtitle: "Please sign in to delete this user",
      };
      return NextResponse.json(response, { status: 401 });
    }
    const { user } = session;
    ...
    if (user.role !== "admin") {
      response.error = {
        title: "Unauthorized",
        subtitle: "You are not authorized to delete this user",
      };
      return NextResponse.json(response, { status: 403 });
    }
