...
import { updateCanvas, deleteCanvas, findCanvasById } from "@/repositories/notes";
  ...
  try {
    ...
    if (!session?.user) {
      return NextResponse.json(
        { error: { title: "Unauthorized", subtitle: "Please sign in to update a canvas" } },
        { status: 401 }
      );
    }
    ...
    if (!userCanvas) {
      return NextResponse.json(
        { error: { title: "Forbidden", subtitle: "You don't have permission to update this canvas" } },
        { status: 403 }
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (!session?.user) {
      return NextResponse.json(
        { error: { title: "Unauthorized", subtitle: "Please sign in to delete a canvas" } },
        { status: 401 }
      );
    }
