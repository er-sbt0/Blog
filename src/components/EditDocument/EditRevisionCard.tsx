...
const RevisionCard: React.FC<{
  ...
  const localRevision = localDocumentRevisions.find((r) =>
    r.id === revision.id
  );
  const isLocalRevision = !!localRevision;
  const cloudRevision = cloudDocumentRevisions.find((r) =>
    r.id === revision.id
  );
  ...
  const showUpdate = isOnline && isDocumentAuthor && isCloudRevision &&
    !isCloudHead;
  const showDelete = !diff.open && isRevisionAuthor && !isLocalHead &&
    !isCloudHead;
  ...
  const createRevision = async () => {
    ...
    if (isLocalDocument && !isCloudDocument) {
      const editorDocument = {
        ...localDocument,
        data: editorDocumentRevision.data,
        revisions: [],
      };
  ...
  const deleteRevision = async () => {
    const alert = {
      ...
      content:
        `Are you sure you want to delete this revision? This will remove it from both cloud and local storage if it exists in both.`,
      actions: [
