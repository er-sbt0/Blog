...
import { ArrowBack, ArrowForward, Folder, Home as HomeIcon } from "@mui/icons-material";
  ...
  return (
    <Box sx={{ mb: 2, overflow: "auto", maxHeight: compact ? 350 : undefined }}>
      <Typography variant={compact ? "h6" : "subtitle1"} gutterBottom>
        {compact ? `Move Document: ${documentName}` : "Select destination folder:"}
      </Typography>

      <Paper variant="outlined" sx={{ p: 1, mb: 2 }}>
        <Breadcrumbs aria-label="directory navigation breadcrumbs" maxItems={compact ? 3 : undefined}>
          {breadcrumbs.map((crumb, index) => {
            const isLast = index === breadcrumbs.length - 1;
            const icon =
              crumb.id === null
                ? <HomeIcon sx={{ mr: 0.5, fontSize: compact ? "0.875rem" : "1rem" }} />
                : <Folder sx={{ mr: 0.5, fontSize: compact ? "0.875rem" : "1rem" }} />;
      ...
      {loading
        ? (
          <Box sx={{ display: "flex", justifyContent: "center", p: compact ? 2 : 3 }}>
        ...
        : (
          <>
            ...
            {compact && (
              ...
              >
                <ListItemButton onClick={onMove} sx={{ borderRadius: 1, py: 1 }}>
                  <ListItemIcon sx={{ minWidth: 36 }}>
                    <Folder color="primary" fontSize="small" />
                  </ListItemIcon>
                  <ListItemText
                    primary={`Move to: ${breadcrumbs[breadcrumbs.length - 1]?.name || "Root"}`}
              ...
              ? (
                <List dense={compact} disablePadding={compact}>
                  {directories.map((directory) => {
                    const dirName =
                      directory.local?.name || directory.cloud?.name || "Directory";
                      ...
                      <ListItem key={directory.id} disablePadding>
                        <ListItemButton
                          ...
                          sx={{
                            ...
                            "&:hover": compact ? { bgcolor: "action.hover" } : undefined,
                            ...(isCurrentParent && { bgcolor: "action.selected" }),
                          }}
                        >
              ...
              : (
                <Box sx={{ p: 2, textAlign: "center" }}>
                  <Typography color="text.secondary" variant={compact ? "body2" : "body1"}>
                    {compact ? "No subfolders found" : "No folders found at this level"}
                  </Typography>
                </Box>
              )}
              ...
              <Box sx={{ mt: compact ? 1 : 2 }}>
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (parentIndex >= 0) onBreadcrumbClick(breadcrumbs[parentIndex].id);
                  }}
                  size="small"
                >
