...
const createMockRevision = (id: string, author: User): CloudDocumentRevision => ({
  id,
  documentId: 'doc1',
  author,
...
export function testEmptyRevisions() {
  ...
  console.assert(result.length === 0, 'Should return empty array for empty revisions');
}
...
export function testExcludesUserIds() {
  ...
  console.assert(!result.find(u => u.id === user2.id), 'user2 should be excluded');
  console.assert(!!result.find(u => u.id === user1.id), 'user1 should be included');
  console.assert(!!result.find(u => u.id === user3.id), 'user3 should be included');
}
...
export function testMultipleExcludedUsers() {
  ...
  console.assert(!result.find(u => u.id === coauthor1.id), 'coauthor1 should be excluded');
  console.assert(!result.find(u => u.id === coauthor2.id), 'coauthor2 should be excluded');
}
...
export function testNullAuthors() {
  ...
  const revisions = [
    createMockRevision('rev1', user1),
    { id: 'rev2', documentId: 'doc1', author: null as any, createdAt: new Date().toISOString() },
    createMockRevision('rev3', user2),
