...
## Issue
Notes appear to be deleted when switching between `npm run dev` and `npm run build + start` environments.

## Root Cause
Notes are stored in **IndexedDB** (database name: `matheditor`, version: 4) which is origin-specific. Each origin (protocol + hostname + port) maintains a separate IndexedDB instance.

- `npm run dev` typically runs on `http://localhost:3000`
- `npm run build + start` runs on a different port (e.g., `http://localhost:3001`)

Since these are different origins, they have completely separate IndexedDB storage. Notes created in one environment are not accessible in the other.
